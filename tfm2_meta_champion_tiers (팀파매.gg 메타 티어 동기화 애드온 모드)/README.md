...
팀파매.gg 대시보드가 만든 메타 점수표를 읽어서 게임 안의 팀별 챔피언 티어표(`Team.champion_tiers`)에 반영하는 전용 모드입니다.
...
`overall` 값이 없거나 파싱할 수 없으면 `tier` 컬럼을 그대로 사용합니다.
...
`tfm2_meta_dashboard\tools\build_meta_data.py`는 대시보드 메타 갱신 시 이 모드의 `champion_tier_policy.tsv`도 같이 생성합니다.
