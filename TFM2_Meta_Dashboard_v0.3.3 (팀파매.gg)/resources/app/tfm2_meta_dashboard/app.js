  ...
  return `<div class="match-list">${rows
    ...
    .map((match) => {
      ...
      return `<button class="match-list-item ${active ? "active" : ""}" data-match="${match.id}">
        ...
        <small>패치 ${match.version} · 경기시간 ${durationLabel(match.durationSec)} · ${match.winner === "blue" ? "블루 승" : "레드 승"}</small>
      </button>`;
    })
    .join("")}</div>`;
}
...
function renderMatchView() {
  ...
  if (!allRows.length) {
    if (DATA.sources.metaExportUsable === false) {
      container.innerHTML = `<p class="notice warning-notice">Meta Exporter 또는 Save Probe가 현재 게임 0.4.7 DB 구조 일부와 맞지 않아 예전 리플레이 export 파일을 무시했어. 0.4.7용 진단 도구가 정상 추출될 때까지 신규 경기 분석은 제한될 수 있어.</p>`;
      return;
    }
    ...
    <div class="match-layout">
      ...
      <section class="match-analysis">
        ...
        <div class="match-title">
          <div>
            <small>Replay #${match.id} · ${matchDateLabel(match)} · 패치 ${match.version} · 경기시간 ${durationLabel(match.durationSec)}</small>
